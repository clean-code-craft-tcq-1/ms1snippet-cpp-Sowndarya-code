...
bool generalValidation(double* values, int numOfValues, typeOfReading type) {
  ...
  double genMaxDelta = (type == SOCReading) ? 0.05 ; 0.1;
...
bool validateSOCreadings(double* values, int numOfValues) {
  ...
  //}
  //return true;
  validationResult = generalValidation(values, numOfValues, SOCReading);
...
bool validateCurrentreadings(double* values, int numOfValues) {
  ...
  //}
  //return true;
  validationResult = generalValidation(values, numOfValues, CurrentReading);
  return validationResult;
}
